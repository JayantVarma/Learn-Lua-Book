...
local tapEvent = Event.MOUSE_UP --Event.TOUCHES_END
...
function setVisible(theObject, setHow)
	...
	theObject:setVisible(setHow or false)
end
...
function updateText(theObject, theNewText)
	if theObject == nil and theObject.setText then 
		return 
	end
	theNewText = theNewText or ""
	theObject:setText(theNewText)
end 
...
function createHUDItems()
	...
	txtLives     = newText(lives,     170, baseLine + 55)
	txtFuel      = newText(fuel,      610, baseLine + 55)
	txtSaved     = newText(collected, 840, baseLine + 55)
	txtScore     = newText(score,     410, baseLine + 55)
...
stage:addEventListener(tapEvent, shoot)